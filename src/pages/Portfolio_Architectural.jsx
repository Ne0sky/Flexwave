...
const Portfolio_Architectural = () => {
    ...
    return (
      <div className='font-primary overflow-x-hidden '>
        <Banner_half title='Architectural'/>
        <Navbar_portfolio/>
        <div className="h-[1px] w-3/4 rounded-xl mx-auto bg-zinc-900 hidden md:flex mb-5"></div>
          ...
          {AutomotiveSrc.map((data, index)=>{
              return  <div key={index}  className='overflow-hidden rounded-xl'><img onClick={()=>setFile(data)} key={index} className='overflow-hidden  transition-all ease-in-out duration-500 hover:scale-105  object-cover w-full cursor-pointer h-auto shadow-lg' src={data.path} alt="Porfolio_img" /></div>
          })}
          </div>
  ...
  export default Portfolio_Architectural